...
\begin{algorithmic}[1]
    \Require{$graph$, $start$, $fn$}
    \Ensure{All nodes in $graph$ get processed with $fn$}

    \algblockdefx[SPAWN]{Spawn}{EndSpawn}{\bf{spawn}}

    \Statef{unvisited \gets \{ start \})}
    \Statef{visited \gets \{\}}
    
    \Spawn
        \While{$unvisited$ not empty}
            \Statef{node \gets unvisited.take()}
            \Statef{visited.put(node)}
            \Statef{fn(node)}
            \For{ $next \in neighbors(node)$ }
                \If{ $next \not\in visited$ }
                    \Statef{unvisited.put(next)}
                \EndIf
            \EndFor
        \EndWhile
    \EndSpawn

    \Statef{wait~for~workers}
\end{algorithmic}
\end{algorithm}
...
\begin{algorithmic}[1]
    ...
    \algblockdefx[SPAWN]{Spawn}{EndSpawn}{\textbf{spawn}}{}

    \Statef{added \gets Semaphore(0)}
    \Statef{terminate \gets false}
    \Statef{mutex \gets Mutex()}
    \Statef{workers \gets 0}
...
We use $mutex$ to protect variables and data structures. 
Semaphore $added$ tracks how many items are in the $unvisited$ set, if the process finally terminates it is turned into a turnstile on line 31 and 13. 
Variable $workers$ tracks how many workers are busy.
